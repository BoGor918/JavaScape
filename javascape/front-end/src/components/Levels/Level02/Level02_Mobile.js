...
export default function Level02() {
  ...
  const { unityProvider, addEventListener, removeEventListener, loadingProgression, isLoaded, sendMessage } =
    useUnityContext({
      loaderUrl: "/Level02-Mobile/Level02-Mobile.loader.js",
      dataUrl: "/Level02-Mobile/Level02-Mobile.data",
      frameworkUrl: "/Level02-Mobile/Level02-Mobile.framework.js",
      codeUrl: "/Level02-Mobile/Level02-Mobile.wasm",
    });
  ...
  const handleGameOver = useCallback((score) => {
    ...
    // add space coin
    updateDoc(updateUserRef, { SpaceCoin: 40 + currentUserDataSet[6] })
    ...
    <Fragment>
      {
        loading ? <Loading /> :
          authUser != null && authUser.emailVerified === false ? <EmailVerification /> :
            isMobile === false ? window.location.replace("/battle/level02") :
